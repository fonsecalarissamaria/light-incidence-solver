...
// Tolerância para tratar quinas e bordas
...
// Função auxiliar interna para interseção paramétrica
// (Resolve problemas de precisão com coordenadas muito grandes)
static bool getSegmentIntersection(const Segment& s1, const Segment& s2, Point& intersection) {
    ...
    // Verifica se interseção ocorre dentro dos segmentos [0, 1] com tolerância
...
int Rectangle::countIntersections(const Segment& ray) const {
    ...
    // Filtra duplicatas (caso o raio bata exatamente na quina)
    int uniqueCount = 0;
